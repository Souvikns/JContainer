...
const createContainer = async (db, container_name) => {
    ...
    const coll = await db.listCollections({name: METADATA}).next()

    if(coll){
        /**
         * First we need to get the _metadata from the database
         */

        let _metadata = db.collection(METADATA).findOne()

        let oId = new ObjectId(_metadata._id)

        let response = await db.collection(METADATA)
            .findOneAndUpdate({_id: oId}, {$push: container_name})
